...
from .config import ImageDimensionsConfig


log = logging.getLogger("mkdocs.material.image_dimensions")

_ASPECT_RATIO_RE = re.compile(
    r"(?:^|;)\s*aspect-ratio\s*:", re.IGNORECASE
)
...
def _local_image_path(src, page, config):
    ...
    if parsed.scheme or parsed.netloc:
        if os.environ.get("DEBUG") == "1" and (
            parsed.scheme in { "http", "https" } or parsed.netloc
        ):
            log.info(
                "Skipping remote image because its dimensions cannot be "
                "determined without downloading it: %s", src
            )
        return None
    if not parsed.path:
        return None
...
def _attribute_index(attributes, name):
    return next((
        index for index, (attribute_name, _) in enumerate(attributes)
...
class ImageTagParser(HTMLParser):
    ...
    def _handle_image(self, attributes, self_closing):
        ...
        if (
            _attribute_index(attributes, "width") is not None and
            _attribute_index(attributes, "height") is not None
        ):
            return
...
class ImageDimensionsPlugin(BasePlugin[ImageDimensionsConfig]):

    def on_page_content(self, html_content, *, page, config, **kwargs):
        if not self.config.enabled:
            return

        parser = ImageTagParser(html_content, page, config)
        parser.feed(html_content)
        parser.close()

        for start, end, replacement in reversed(parser.replacements):
            html_content = (
                html_content[:start] + replacement + html_content[end:]
            )
        return html_content
